...
| **Exclude existing annotations** | (Only visible when **Interactive preannotations** is enabled)<br /><br />When enabled, only the tool-created prompt region is sent to the ML backend during interactive preannotation. Preexisting annotations on the task are excluded from the request context. <br /><br />This is useful when working with large or complex annotations (such as brush masks) that are expensive to serialize. Excluding them can significantly reduce client-side processing time with no loss of prediction quality for prompt-based models such as SAM2. |
