...
| **Exclude existing annotations**         | (Available when **Interactive preannotations** is enabled)<br /><br />When enabled, only the tool-created prompt region is sent to the ML backend during interactive preannotation. Preexisting annotations on the task are excluded from the request context. <br /><br />Useful for prompt-based models (e.g. SAM2) when tasks contain large preexisting annotations such as brush masks, which are expensive to serialize client-side.   |
